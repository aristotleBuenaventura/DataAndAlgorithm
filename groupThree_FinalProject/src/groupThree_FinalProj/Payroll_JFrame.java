 ...
 * Payroll_JFrame
 * 
 * This serves as the system's main window page. Array, Queue, Stack, ArrayList,
 * and Binary Tree data structures and algorithm were all used in this section,
 * as well as the calculation of employee salaries. In this class, the admin user 
 * can add, delete, clear, update, sort, and search employee records.
 * 
 */
...
public class Payroll_JFrame extends JFrame {
	...
	//ArrayList
	private static ArrayList<EmployeeDto> tableColumn = new ArrayList<EmployeeDto>();
	//Queue
	...
	{
		model.setRowCount(0);
		for (EmployeeDto employeeInfo : tableColumn) {
			Object[] row = new Object[13];
			getRow(employeeInfo, row);
			model.addRow(row);
		}
	}

	//For sorting purposes
	private void getRow(EmployeeDto employeeInfo, Object[] row)
	{
		row[0]= employeeInfo.getId();
		row[1]= employeeInfo.getName();
		row[2]= employeeInfo.getField();
		row[3]= employeeInfo.getWorkingHours();
		row[4]= employeeInfo.getBasicPay();
		row[5]= employeeInfo.getOtp();
		row[6]= employeeInfo.getPhilHealth();
		row[7]= employeeInfo.getPagIbig();
		row[8]= employeeInfo.getSss();
		row[9]= employeeInfo.getTax();
		row[10]= employeeInfo.getGross();
		row[11]= employeeInfo.getDeduction();
		row[12]= employeeInfo.getNet();
	...
	public Payroll_JFrame() {
		...
		textFieldID.setBounds(203, 14, 150, 21);
		...
		fieldComboBox.setBounds(203, 42, 150, 21);
		contentPane.add(fieldComboBox);
		fieldComboBox.addItem("Manager");
		fieldComboBox.addItem("Accountant");
		fieldComboBox.addItem("Technician");
		fieldComboBox.addItem("Instructor");
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(i>=0) {
					...
					model.setValueAt(fieldComboBox.getSelectedItem(), i, 2);
					...
					model.setValueAt(textFieldSSS.getText(), i, 8);
					model.setValueAt(textFieldTax.getText(), i, 9);
					model.setValueAt(textFieldGross.getText(), i, 10);
					model.setValueAt(textFieldTotalDeduction.getText(), i, 11);
					model.setValueAt(textFieldNet.getText(), i, 12);

					b.update(textFieldName.getText(),textFieldID.getText(),fieldComboBox.getSelectedItem().toString());
				}else {
					JFrame errorFrame = new JFrame();
					Toolkit.getDefaultToolkit().beep();
					JOptionPane.showMessageDialog(errorFrame, "Please Select a Row First","ERROR", JOptionPane.ERROR_MESSAGE);
		...
		//transfer data to the table using setText
		...
		tablePayRoll.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(i>=0) {
					...
					fieldComboBox.setSelectedItem(model.getValueAt(i,2).toString());
					...
					textFieldSSS.setText(model.getValueAt(i,8).toString());
					textFieldTax.setText(model.getValueAt(i,9).toString());
					textFieldGross.setText(model.getValueAt(i,10).toString());
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(textFieldID.getText().equals("") || textFieldName.getText().equals("") ||  textFieldPhilHealth.getText().equals("")|| textFieldWorkingHours.getText().equals("") ||  textFieldGross.getText().equals("") ||  textFieldNet.getText().equals("") ||  textFieldTax.getText().equals("") ||  textFieldTotalDeduction.getText().equals("") 
					...
					JFrame errorFrame = new JFrame();
					Toolkit.getDefaultToolkit().beep();
					JOptionPane.showMessageDialog(errorFrame, "Please Fill Complete Information","ERROR", JOptionPane.ERROR_MESSAGE);
					...
					//store to array list
					employee.setId(textFieldID.getText());
					
					//Capitalized Each Word
					String empName = textFieldName.getText();
					String capitalizedName = Arrays.stream(empName.split("\\s+"))
						.map(name -> name.substring(0, 1).toUpperCase() + name.substring(1)) 
						.collect(Collectors.joining(" "));
					
					employee.setName(capitalizedName);
					employee.setField(fieldComboBox.getSelectedItem().toString());
					employee.setWorkingHours(textFieldWorkingHours.getText());
					employee.setBasicPay(textFieldBasicPay.getText());
					employee.setOtp(textFieldOTP.getText());
					employee.setPhilHealth(textFieldPhilHealth.getText());
					employee.setPagIbig(textFieldPagIbig.getText());
					employee.setSss(textFieldSSS.getText());
					employee.setTax(textFieldTax.getText());
					employee.setGross(textFieldGross.getText());
					employee.setDeduction(textFieldTotalDeduction.getText());
					employee.setNet(textFieldNet.getText());

					tableColumn.add(employee);
		...
		btnOK.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(textFieldID.getText().equals("") || textFieldName.getText().equals("") || textFieldWorkingHours.getText().equals("")){
					...
					// Formulas // 
					int rateHour;
					if(fieldComboBox.getSelectedItem().toString().equals("Manager")) {
						rateHour=72;
					} else if(fieldComboBox.getSelectedItem().toString().equals("Accountant")) {
						rateHour=78;
					} else if(fieldComboBox.getSelectedItem().toString().equals("Technician")) {
						rateHour=65;
					} else {
						rateHour=24;
					}
					...
					int basicPay = rateHour *numHrsWork;
		...
		lblSearch.setFont(new Font("Consolas", Font.BOLD, 18));
		lblSearch.setBounds(812, 16, 73, 21);
		contentPane.add(lblSearch);

		textFieldSearch = new JTextField();
		textFieldSearch.setColumns(10);
		textFieldSearch.setBounds(893, 15, 93, 21);
		contentPane.add(textFieldSearch);

		//Sorting categories combo box 
		JComboBox<String> sortingComboBox = new JComboBox<String>();
		sortingComboBox.setBounds(893, 81, 93, 21);
		contentPane.add(sortingComboBox);
		sortingComboBox.addItem("ID");
		sortingComboBox.addItem("Name");
		sortingComboBox.addItem("Field");
		sortingComboBox.addItem("Working Hours");
		sortingComboBox.addItem("Basic Pay");
		sortingComboBox.addItem("OTP");
		sortingComboBox.addItem("PhilHealth");
		sortingComboBox.addItem("Pag-ibig");
		sortingComboBox.addItem("SSS");
		sortingComboBox.addItem("Tax");
		sortingComboBox.addItem("Gross");
		sortingComboBox.addItem("Deduction");
		sortingComboBox.addItem("Net");
		...
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(textFieldSearch.getText().equals("")) {
					JFrame errorFrame = new JFrame();
					Toolkit.getDefaultToolkit().beep();
					JOptionPane.showMessageDialog(errorFrame, "Please Enter the Correct Name","ERROR", JOptionPane.ERROR_MESSAGE);
				}else {
					b.search(textFieldSearch.getText().toString().toLowerCase());
				}

			}
		});
		btnSearch.setBounds(893, 41, 93, 21);
		contentPane.add(btnSearch);

		//Sorting 
		JButton btnSortBy = new JButton("SORT");
		btnSortBy.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {

				String type = "string";
				switch(sortingComboBox.getSelectedItem().toString()) {
				...
				default:
					...
					break;
														...
														//Name,"String"
				QuickSort sort = new QuickSort(sortingComboBox.getSelectedItem().toString(), type);
				sort.sort(tableColumn, 0, tableColumn.size() - 1);

				refreshTable();
			}
		});

		btnSortBy.setBounds(893, 113, 93, 21);
